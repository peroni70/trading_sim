...
(** [rule] represents all the elements of a valid trading rule in relation to a
    specific [stock]. All these elements are listed below. "all" is represented 
    as [amt] = -1 *)
type rule = {
  verb: string; (* command *)
...
let get_init_state path stocks =
  ...
  Ui.make_state stocks balance portfolio short_pos value start_date [] 
...
let execute_rule state rule dir =
  ...
  if comp (evaluate_measure rule.measure rule.argument 
             (Trade.get_ticker dir (String.uppercase_ascii rule.ticker)) state) rule.number then 
...
(** [execute-trades state rule_lst valid stocks] iterates through the list of
    rules in play by the simulation and returns a state with any trades executed
    if their rule condition is true. Also updates the global [rule_lst] if the 
    rule in question is a "once" rule and has fired for the first time. *)
...
(** [step state stocks] advances to the next day in the simulation, creating a
    new state and advancing the date by one. If the simulation ends, prints
    the user's results. *)
...
(** [main_sim ()] intializes the simulator, parsing the user inputted stock data
    and trade rules file into their relevant types. *)
let main_sim () = 
...
(** Initializes the game, printing the jane street logo and prompting the user 
    to choose which tool they want to use*)
let rec pre_main () =
